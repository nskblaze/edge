...
//			bq24296_set_charge_current(CHARGE_CURRENT_2048MA);
			bq24296_set_charge_current(CHARGE_CURRENT_1024MA);		//Modified by nsk
		...
		const struct i2c_device_id *id)
{
	...
	/*Setting Charge current to 1A*/
	//bq24296_set_charge_current(CHARGE_CURRENT_512MA);

	/* Disable chagrer */
	//bq24296_update_charge_mode(CHARGE_MODE_CONFIG_CHARGE_DISABLE); //disabled by nsk for default charging mode.
	
	/*Enable Charger*/
	bq24296_update_charge_mode(CHARGE_MODE_CONFIG_CHARGE_BATTERY); 	//enabled by nsk for default charging mode.
